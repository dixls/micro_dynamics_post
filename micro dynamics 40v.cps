...
description = "Micro Dynamics Tera 40V";
vendor = "Mitsubishi";
...
longDescription = "Generic post for Micro Dynamics Mega 30VT with a Mitsubishi M830W control with 5-axis capabilities. Modified for Tera 40V for Engin Cycles";
...
*/
var firstNote = true; // ignore onParameter call for first job note, it is handled in onOpen
function writeNotes(_note) {
  if (!getProperty("showNotes")) {
    return;
  }

  var notes;
  if (_note == "job-notes") { // 1st setup notes
    if (hasGlobalParameter(_note)) {
      notes = getGlobalParameter(_note);
    }
  } else if (_note == "notes") { // operation notes
    if (hasParameter(_note)) {
      notes = getParameter(_note);
    }
  } else { // 2nd thru nth setup notes
    if (firstNote) {
      firstNote = false;
      return;
    }
...
function onOpen() {
  ...
  smoothing = getProperty("useSmoothing");
  gSmoothing = getProperty("useSmoothingType") == "II" ? 2 : getProperty("useSmoothingType") == "III" ? 3 : 1;
  rSmoothing = 0;

  if (getProperty("fiveAxis")) { // note: setup your machine here
  ...
  if (forceMultiAxisIndexing || !is3D() || machineConfiguration.isMultiAxisConfiguration()) { // use 5-axis indexing for multi-axis mode
    ...
    if (_section.isMultiAxis()) {
      ...
      if (useMultiAxisFeatures) {
        ...
        abc = getWorkPlaneMachineABC(_section.workPlane, true, true);
      }
      if (_setWorkPlane) {
        setWorkPlane(abc);
      }
...
function getTcpCode(section) {
  // assumes a Head configuration uses TCP on controller
  var offsetCode = 43;
  if (section.isMultiAxis()) {
    if (machineConfiguration.isMultiAxisConfiguration() && (section.getOptimizedTCPMode() == 0)) {
      offsetCode = 43.4;
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || forceSmoothing || toolChecked) {
  ...
  if (workOffset == 0) {
    error(localize("Work offset has not been specified. Please select a work offset.")); // Generates an error if work offset is set to 0 rather than defaulting to 1
    //workOffset = 0;
  }
  ...
  if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    offsetCode = getTcpCode(currentSection);
...
function onRewindMachine(_a, _b, _c) {
  ...
  // Reinstate TCP
  writeBlock(gFormat.format(getTcpCode(currentSection)), hFormat.format(tool.lengthOffset), formatComment("TCPC ON"));
...
function onSectionEnd() {
  ...
  if (currentSection.isMultiAxis()) {
    ...
    if (!hasNextSection() || !getNextSection().isMultiAxis()) {
      clampAxis(currentSection.getFinalToolAxisABC(), true, false);
    }
  }
}
...
function clampAxis(abc, clamp, force) {
  if (!getProperty("useClampCodes")) { // clamp codes are not supported
    return;
  }

  if (clamp) { // clamp axes
    if (!aAxisIsClamped || force) {
      writeBlock(mFormat.format(68), formatComment("CLAMP A-AXIS"));
      aAxisIsClamped = true;
    }
    if (!cAxisIsClamped || force) {
      if (cOutput.isEnabled()) {
        writeBlock(mFormat.format(10), formatComment("CLAMP C-AXIS"));
        cAxisIsClamped = true;
      }
    }
  } else { // unclamp axes
    ...
    if (currentSection.isMultiAxis()) {
      ...
      unclampC = cAxisIsClamped && abcFormat.areDifferent(abc.z, cOutput.getCurrent());
    }
    if ((aAxisIsClamped && unclampA) || force) {
      writeBlock(mFormat.format(69), formatComment("UNCLAMP A-AXIS"));
      aAxisIsClamped = false;
    }
    if ((cAxisIsClamped && unclampC) || force) {
      if (cOutput.isEnabled()) {
        writeBlock(mFormat.format(11), formatComment("UNCLAMP C-AXIS"));
...
function onClose() {
  ...
  writeBlock(
    gFormat.format(30), "P3", gAbsIncModal.format(91), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0)
    );
